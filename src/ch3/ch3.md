...
- 병행 프로그램의 '올바른' 결과는 여러 개일 수 있다. 그러므로 입력에 따라 정확한 답이 결정되는 *함수*가 아니라 *관계(relation)*를 나타내는 것이라 보는 게 옳다. 병행성에는 *비결정성(non-determinism)*이 따라올 수 밖에 없다.
...
- 함수 방식을 따르더라도 시간과 관련된 문제들이 생겨날수 있음을 알게 된다. 그 가운데 특히, 두 물체 사이에서 오고가는 일을 흉내내려고 할 때, 말하자면 *대화형 시스템interactive system*을 설계할 때 훨씬 까다로운 문제가 생긴다.
- 두 트랜잭션(스트림)을 요청하는 두 스트림을 엮는 방식으로 단순히 요청을 번갈아 섞는 방법으로 두 스트림을 합쳐서는 안 되기 때문이다. 그러므로 `merge`를 만든다 치면 시간을 정해 놓고 그 전에는 어떤 트랜잭션이 처리되고 그 후에 처리할 트랜잭션은 어떤 것인지 서로 동의한 것처럼 어떻게 해서든 '실시간real time'으로 일이 처리되는 듯 받아들이도록 두 트랜잭션 스트림을 알맞게 뒤섞는 방법이 필요하다.
- 따라서 함수 방식에 따라 이 문제를 풀기로 하였다면, 어쩔 수 없이 서로 다른 에이전트agents에서 들어오는 입력을 한데 섞어야 하고 그 때문에 함수 방식을 지킴으로써 사라질 것이라 믿은 문제점들을 다시 끌어들이게 된다. 이런 제약은 동시성 문제점에서 다룬 제약과 똑같다.
- 어떤 관점(물체 또는 함수 사고방식)을 따르든 제각기 큰 장점을 얻을 수 있지만, 어느 한 쪽도 완전히 만족스러운 해법을 안겨 주지는 못한다. **이 둘을 멋지게 합치는 방법은 아직도 나오지 않았다.**
