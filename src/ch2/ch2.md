...
- 이 `dispatch` 프로시저가 새로 만든 환경을 들고 다닌다고 보면 된다.
  - 이처럼 '프로시저'나 '데이터' 역할이 쉽게 뒤바뀐다는 게 놀랍다. 그러므로, 여기서 꼭 짚고 넘어가야 할 것은, 프로시저를 물체처럼 쓸 수 있는 언어에서는 프로시저와 물체 사이에 큰 차이점이 없다는 사실이다.
...
- 타입의 계층 관계hierarchy of types:
  ...
  - `drop`: 탑 구조에서 한 데이터 타입 물체를 가장 단순한 표현으로 '끌어내리는lower'일을 한다.
